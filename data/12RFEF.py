...
def get_file_path():
    # Ruta local
    local_path = r"C:\Users\rricobaldi\Desktop\OPTA - Provision\Informes Power BI\Ligas\1 RFEF\Futboleras\data\1-2 RFEF 23Enero2025.xlsx"
    
    # Verificar si el archivo existe en la ruta local
    if os.path.exists(local_path):
        return local_path
    else:
        # Si no existe en la ruta local, usar la ruta relativa (para Streamlit Sharing)
        return "1-2 RFEF 23Enero2025.xlsx"
...
def load_data():
    try:
        ...
        st.error("Error: El archivo Excel no se encontró. Asegúrate de que el archivo esté en la ruta correcta.")
        return pd.DataFrame()
