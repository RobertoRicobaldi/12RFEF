...
def get_file_path():
    # Ruta relativa al archivo Excel (dentro de la carpeta 'data')
    return "data/1-2 RFEF 23Enero2025.xlsx"
...
def load_data():
    try:
        ...
        if not os.path.exists(file_path):
            st.error(f"Error: El archivo '{file_path}' no se encontró. Asegúrate de que esté en la ruta correcta.")
            return pd.DataFrame()
