...
def get_file_path():
    ...
    if os.path.exists(local_path):
        st.write("Cargando archivo desde la ruta local...")
        return local_path
    else:
        st.write("No se encontró el archivo local. Intentando cargar desde GitHub...")
        # Si no existe en local, intentar cargar desde GitHub
        response = requests.get(github_url)
        if response.status_code == 200:
            st.write("Archivo cargado correctamente desde GitHub.")
            return github_url
        else:
...
def pagina_filtros():
    ...
    st.write(df_filtrado.head(10))  # Mostrar 10 filas del DataFrame filtrado
...
def pagina_busqueda():
    ...
    st.dataframe(df_filtrado.head(10))  # Mostrar 10 filas con las mismas columnas que la vista previa
